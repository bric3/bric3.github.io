...
==== Inspecting memory mappings which memory is resident
...
00007ff9b91e7000      16       0       0 -----   [ anon ] <3>
00007ff9b91eb000    1012      24      24 rw---   [ anon ] <4>
00007ff9b92e8000      16       0       0 -----   [ anon ]
00007ff9b92ec000    1012      92      92 rw---   [ anon ]
...
[source, role="secondary"]
----
I have no name!@edge-api-769ccdd5f7-65rnq:/$ cat /proc/7/maps | head -n 20
...
+
The `16 KiB` are the thread guard pages (they are used when a stack overflow error happens,
their permission will change in order to handle the error) ; read
...
The first thing we did is to ignore _file-backed_ memory mapping. Yet
these memory mappings also consumes pages.
...
and as such may deserve some attention if RSS usage is high but the app memory alone is not
enough.
...
actually resident. This app is OK, but I've seen in the past `FileChannel`
mappings not well released, leading to increasing memory consumption, that weren't
part of the Java heap. Other applications may just work with a lot of files.
