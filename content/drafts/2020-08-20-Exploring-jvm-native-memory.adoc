...
// TODO: Mention NMT baseline summary.diff
...
us to be more careful with memory. The cgroups limit is a double edged
sword, it ensures sibling containers won't be affected by a misbehaving
application, however it may be harder to configure correctly, resulting
in particular in _oomkills_ or inefficient memory settings thus affecting
negatively the cost and pod scheduling on a Kubernetes cluster. And even memory
settings that seems good but too tight may have bad implications on
application performance.

Java applications are trickier to get right this limit, and I think the
`MaxRAMPercentage` flag is not the right tool to do this job.
...
To answer these questions this entry is about figuring out the native memory
of a `java` process.
