...
[source,bash]
----
env -u JAVA_TOOL_OPTIONS jcmd $(pidof java) \
  VM.log \
    what="gc*,gc+heap=debug,gc+ref=debug,gc+ergo*=trace,gc+age*=trace,gc+phases*,safepoint*,os*,gc+region=trace,gc+liveness=trace" \
