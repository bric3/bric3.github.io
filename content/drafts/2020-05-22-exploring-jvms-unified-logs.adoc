...
| breakpoint            | `jvmti` (Redefinition)         | Part of the JVMTI, but logged under `redefine` _log tag root_.
| bytecode              | `jfr`                          |
...
| dcmd                  | `jfr`                          | Diagnostic command, apparently only used in combination with the _log tag root_ by `jfr`
...
| exceptions            |                                | About Java exceptions handling in different parts (interpreter, during safepoint, compiler, ...). Specific exception handling during redefinition when combined with `redefine`.
...
| oopmap                | `interpreter`, `redefine`      | Ordinary Object Pointer caching logs, combined with by `interpreter` and `redefine`.
