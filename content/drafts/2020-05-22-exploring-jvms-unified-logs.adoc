...
to update the logging configuration when changing the GC algorythm, e.g. using CMS then switch to G1GC.
...
.Reload `Foo` class after instantiation during debug
[source,shell]
----
$ java -Xlog:redefine*=debug,jvmti*=trace -agentlib:jdwp=transport=dt_socket,server=n,suspend=y,address=61222 -cp .:./lib/byte-buddy-agent-1.10.13.jar:./lib/byte-buddy-1.10.13.jar AgentTest
[83.772s][debug][redefine,class,load] loading name=sandbox.AgentTest$Foo kind=102 (avail_mem=1379732K)                  <1>
[83.772s][info ][redefine,class,constantpool] old_cp_len=25, scratch_cp_len=25                                          <1>
[83.772s][debug][redefine,class,constantpool] after pass 0: merge_cp_len=25                                             <1>
[83.772s][debug][redefine,class,constantpool] after pass 1a: merge_cp_len=27, scratch_i=25, index_map_len=2             <1>
[83.772s][info ][redefine,class,constantpool] merge_cp_len=27, index_map_len=2                                          <1>
[83.772s][debug][redefine,class,load        ] loaded name=sandbox.AgentTest$Foo (avail_mem=1379732K)                    <1>
[83.772s][debug][redefine,class,load        ] loading name=sandbox.AgentTest kind=102 (avail_mem=1379732K)              <1>
[83.772s][info ][redefine,class,constantpool] old_cp_len=176, scratch_cp_len=176                                        <1>
[83.772s][debug][redefine,class,constantpool] after pass 0: merge_cp_len=176                                            <1>
[83.772s][debug][redefine,class,constantpool] after pass 1a: merge_cp_len=176, scratch_i=176, index_map_len=0           <1>
[83.772s][info ][redefine,class,constantpool] merge_cp_len=176, index_map_len=0                                         <1>
[83.772s][debug][redefine,class,load        ] loaded name=sandbox.AgentTest (avail_mem=1379732K)                        <1>
[83.773s][info ][redefine,class,update      ] adjust: name=sandbox.AgentTest$Foo                                        <1>
[83.773s][debug][redefine,class,update,constantpool] cpc special, static or dynamic entry update: <init>(()V)           <1>
[83.773s][info ][redefine,class,load               ] redefined name=sandbox.AgentTest$Foo, count=1 (avail_mem=1379732K) <1>
[83.773s][info ][redefine,class,load               ] redefined name=sandbox.AgentTest, count=1 (avail_mem=1379732K)     <1>
[83.774s][info ][redefine,class,timer              ] vm_op: all=0  prologue=0  doit=0                                   <1>
[83.774s][info ][redefine,class,timer              ] redefine_single_class: phase1=0  phase2=0                          <1>
[83.779s][debug][redefine,class,breakpoint         ] setting breakpoint in main(([Ljava/lang/String;)V)                 <1>
[201.432s][debug][redefine,class,breakpoint         ] clearing breakpoint in main(([Ljava/lang/String;)V)               <2>
[201.433s][debug][redefine,class,breakpoint         ] setting breakpoint in main(([Ljava/lang/String;)V)                <2>
...                                                                                                                     <3>
----
<1> Reload the class `Foo` after it's instantiation, breakpoint at the second line of the `main` method.
<2> Resumes the normal execution flow.
<3> The rest of the program output.
...
[source,shell]
----
$ java -Xlog:jit*=debug,compilation*=debug -cp .:./lib/byte-buddy-agent-1.10.13.jar:./lib/byte-buddy-1.10.13.jar AgentTest
...
...
And, in that regard I believe the new system lacks good default options for logging.
...
In general, I think unified logging suffers from few drawbacks:
...
but there are still some old `Trace*` flags that setup unified logging ; I think
JDK maintainers should have kept the old flags or proposed alternative flags for GC.




This article has been written using the code base of JDK11u 11.0.8. Not everything is
logged via the unified logging mechanism.

For example if you need to work with the JVM compiler for some reason then
`-Xlog:compilation*=debug` might not be enough, instead you'll need to use
`-XX:+LogCompilation`, which by default will generate a file like `hotspot_pid{pid}.log`
unless `-XX:LogFile=compilation-log.xml` is specified. And the content of this file is
actually an XML document.
