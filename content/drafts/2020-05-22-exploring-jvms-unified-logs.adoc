...
== Explore unified logging in a useful way
...
Note the below snippet splits the log multiple lines and add spaces for clarity.
...
Note that some options do not have equivalent in unified GC logging.
...
Now it's the right opportunity to warn a about the caveats of this log configuration.
This configuration is fine and work reasonably well, BUT this configuration
actually misses information like IHOP, which is not only logged with the `ergo` tag
as we'll see.


.exhaustive translation table with some caveats
