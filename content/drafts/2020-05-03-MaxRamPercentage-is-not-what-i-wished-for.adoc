...
Now JDK 11 does this by default and reads information from `cgroups` and
...
In doing so the following writing tries to gather various pieces of information
...
various ways, e.g. using `ps` or reading the `/proc` should give the same number
...
<7> This is app is using G1GC, this section contains `GC` internal data structures which takes ~`225 MiB`
<8> C1 / C2 compilers (which compile bytecode to assembly) uses ~`5.8 MiB`
<9> The `Symbol` section contains many things like interned strings and other internal constants ~`28.2 MiB`
...
*resident* and not accounted for by the RSS counter.
...
the memory limit was increased. I don't have anything to back that idea except how I expect these memory arenas
to grow but not the others.

1. The thread stack memory arena, the increased actual memory pages is small, but enough to be mentioned.
