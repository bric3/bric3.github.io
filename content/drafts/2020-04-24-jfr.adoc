...
r.stop(); // <5>
r.dump(Files.createFile("/var/jfr/app-initiated.jfr")); // <6>
----
...
Now the API allows to read emiited `.jfr` files. The API represents what's actually
in a file, a schema of the events and the events themselves.

.Read JFR files yourself
[source, java]
----
try(RecordingFile rf = new RecordingFile(Paths.get("/var/jfr/app-initiated.jfr"))( { // <1>
    // read the schema
...
`RecordingFile` api is a bit akward to work with, more specifically parsing each events
requires to look at the event descriptor (via `getEventType()`, or getFileds()), and interrogate
the event as fields presence may evolve with each JDK revision. The javadoc advises to
program defensively on that regard :
...
This API is complemented by streaming live events in JDK 14, but that's out of scope
for this article.

While such facilities are useful especially as custom Spring Actuators. When there are
not around or they are just too late the most actionable way to get recording is from
the command line.


== How and what to profile in production

The next items will present command line flags to setup JFR.
...
// XXX
// XXX
// TODO READ https://docs.oracle.com/en/java/javase/11/troubleshoot/troubleshoot-performance-issues-using-jfr.html#GUID-0FE29092-18B5-4BEB-8D8D-0CBA7A4FEA1D
...
briefly presents the other elephant in the room (in a positive way), JDK Mission Control
...
JDK Mission Control, is the tool that will make sense of all these events.
...
user interface evolved quite a bunch. Concretely following the official
...
to learn how to inspect a recording you may be a bit lost.
