...
CAUTION: Change _memory zone_ in NMT to _memory by JVM components_

CAUTION: Change JDK links to openjdk/jdk
...
I will use as a specimen one of the issue that I experienced in production, the actual
problem could be solved without diving in memory, but other factors served as an excuse
...
=== The JVM flags as a starting points
...
The Java heap is only a part of the process memory usage. So now let's dig into how memory
is _consumed_. The values or snippet comes from an application running inside a container.
...
a pretty good picture of the "cost" of running code the JVM.
...
the memory usage of the JVM component with a _baseline_. However, *it doesn't answer
what is actually accounted in the RSS column of `ps`*.

==== Let's pause a bit and revise OS memory management
...
The graph above does not yet show resident memory. Indeed, the above graph most
...
big block of memory.
The OS is hard at work performing various tricks to keep this illusion for all processes.
Since programs run concurrently, **not all memory pages is used at the same time**.
From this observation it's possible :
...
* to use a slower device to store pages, usually a disk, in a special place called _swap_.
...
of the memory. How does it work concretely?
