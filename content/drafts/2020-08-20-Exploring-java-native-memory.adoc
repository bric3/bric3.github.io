...
Using the *committed* value from the NMT output above, and the *mapping size*
this breaks out as :
...
`5576205 KiB` is what this container is supposedly actually using,
as expected this is way over the RSS (`4701120 KiB`) and, also over the
`5 GiB` (`5242880 KiB`) of the pod limit. Due to paging this pod may not have
access all pages especially for a big enough heap. A pod like this is healthy
and can stay that way if the RSS don't grow out that much.
...
0
----
<1> Total size of the adressing space, in other words it's the virtual size = 6507368 KiB
...
A process like may grow it's memory usage up to `1626842` pages.
If the kubernetes memory limit is `5 GiB` (`5242880 KiB`) then
the process will be oom-killed around `1310720` used pages.
