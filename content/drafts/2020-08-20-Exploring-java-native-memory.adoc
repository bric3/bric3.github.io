...
That being said the Java heap is only a part of the process memory usage.
...
.memory zones
[ditaa,"memory-zones",opts=inline]
----
...
[ditaa,"memory-paging",opts=inline]
----
...
Most of the time using `ps` is enough. If you need to understand a bit more
what in the JVM is actually consuming memory, then read on.


The JVM components reported by NMT use different type of memory management and
have different allocation patterns.

. GC based
`Java heap` and `Metaspace` (`Class`) are s=usually the biggest customer of memory,
...
----
+
The interesting part is that these parts rely on the GC algorithm to
_arrange_ the memory. In particular these zones have very large memory segments
that matches the JVM options.
+
Some option may trigger specific behavior for these
memory zones, e.g. enabling the heap to grow or shrink (I never saw that in practice,
maybe I'll see it once I use a JDK 12+ with https://openjdk.java.net/jeps/346[JEP-346],
although even the JEP mention if there is very low activity, which is not gonna
