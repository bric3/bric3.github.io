...
// TODO: Drawing about GC and heap and virtual mem
...
usually of `4 KiB`.
_The are other page sizes and these sizes may even co-exist (e.g. having pages of
...
Nothing. We'll see in a later section how to see how much these files
...
// TODO: redo
image::/assets/off-heap-recon/os-memory-paging.svg[align="center", title="Simple overview of OS paging"]
...
By how much, that depends. I believe page faults is a good indicator.
...
//In this very long writing, I showed two main tool to survey the native
//memory of a process. I understand this knowledge seems
//contradictory to what managed runtimes like Java sell. The reality
//is that our applications runs on finite resources, on Linux as well
//as other OSes, the virtual memory and the scheduling allowed
//processes to share the resources.
//
//With containers the application are guaranteed to have
//the resource they are given (supposing only runs in the cgroup).
//But in order to run many workloads the usage limits got tighter.
//This has the benefit of increasing the density of deployment, by
//maximizing the resource usage.
//
//Meanwhile

A cgroup is a double-edged sword, it constrains misbehaving application to avoid
...
the help of `pmap` complement NMT well. They help to sort out memory
problems of an application or if the limit should just be adjusted.

I understand this may look superfluous and almost zealous to
look at memory segments when coming from the Java world.
