...
.TODO
- [ ] pmap output MemorySegment
- [ ] add-modules, foreign.restricted=permit, Gradle
- [ ] Compare jextract with handcrafted
- [ ] Closing words, easy to implement fall back mechanism (in case of ABI mismatch)
- [ ] Link to the panama mailing list
...
`jextract` is however reading the headers, so it actually extracts
...
THe originating code is here:

.sodium_h_constants_0
[source, java]
----
...
Of course this value is incorrect ; the libsodium path or name is highly
dependent on the installation and the system. It's the same for almost
every shared library (libgit2, libcurl, etc.)

Also, this code is `static final`, which immediately requires a developer
...
without a significant refactoring of the generated code.


As `RuntimeHelper::libraries` is able to load libraries from path, I'll just pass
...
----
static final LibraryLookup[] LIBRARIES = RuntimeHelper.libraries(new String[] {
    "/usr/local/opt/libsodium/lib/libsodium.23.dylib", <1>
});
----
...
The IDE might suggest a method named `crypto_box_keypair$MH`, in fact the suffix
...
More specifically and at this this time `jextract` declares the `MethodHandle`
this way
