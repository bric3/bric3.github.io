...
title: "So MaxRamPercentage isn't enough"
draft: true
#_build:
#  list: never
---
...
Anyway after all, I don't think `*RAMPercentage` flags are very useful (or those
are used inadequately in our application ?!). For me they don't quite respect the _principle of
the least surprise_. As we've seen these percentages do not account how non-heap usage grow,
and the JVM didn't limit these zones according to the `cgroup` limits, which is confusing, because
if they were, we would have seen``OutOfMemoryError``s from these zones.

All things considered, it is actually just as ok, if not more useful to prefer the usual `-Xmx` flags
in a container for now, especially with the `JAVA_TOOL_OPTIONS` environment variable, so you won't
have to calculate a percentage and just use the actual max memory.
...
* Using `Xmx` in a container is still a very good bet compared to `MaxRAMPercentage`
