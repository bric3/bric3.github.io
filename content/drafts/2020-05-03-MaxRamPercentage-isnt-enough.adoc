...
At this time what we are looking for are the process's memory mapped (shared) files:
...
helped me with the above command.
...
but also over the 5 GB (5242880 KB) of the pod limit. Yet this pod is not oom killed.
...
6:   /usr/bin/java -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Dcom.sun.management.jmxremote.port=7199 -Dcom.sun.management.jmxremote.rmi.port=7199 -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.man
Address           Kbytes     RSS   Dirty Mode  Mapping
0000000000400000       4       4       0 r-x-- java
0000000000600000       4       4       4 r---- java
0000000000601000       4       4       4 rw--- java
...
== Going back to choose a wise value for the RAM percentage
...
But this percentage is wrong, if he JVM needed all that memory then the container/pod would
have been _oom killed_. Also, it is necessary to give some free space in the container
...
As a reminder this application was set up with 85% limit for the java heap when the
deployment limit was `3 GB`, it worked under 50% of the traffic but failed with full traffic.
Then tis pod memory limit was bum ped to 5 GB. So actually this is the memory limit that
is a lucky guess for 5 GB, as the RAM percentage was set in the `CMD` directive of the Dockerfile.

This can be explained by two arenas whose usage increased especially, the stacks and the heap usage.

But this application worked well under way less memory. Let's explore that.

=== Actual Java Heap usage

While the previous section allowed to understand actual memory usage, it didn't gave any figure
...
Successive execution may of course indicate an evolving output about java heap usage:
...
be lower at the cost of higher GC activity (and most likely CPU usage).
