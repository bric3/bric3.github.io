...
is *Fedora release 35* with the kernel *5.16.20-200.fc35.x86_64*.
...
The actual syscall numbers (for 32-bit programs) are usually defined in `/usr/include/asm/unistd_64.h`
========
...
That's it; we've made out the first direct syscall using FFM-API.
...
So to make the whole thing work, we need to ake sure the OS has the `memfd_secret`
syscall available.
...
or newer plus LLVM/libclang. In my experience using a locally installed LLVM may
not fit well the single property `llvm_home` as paths to LLVM / clang can be
spread to the Linux distribution choices; instead I would prefer to download
a pre-built binary from https://github.com/llvm/llvm-project[llvm-project],
 –- note it's huge, ~1.6 GiB minimum.
====
...
sh ./gradlew \
  ...
  clean verify # <4>
----
<1> Install a JDK that runs Gradle, currently Gradle 8.11.1 cannot run on Java 25.
<2> Install a recent JDK with `jmods` (those are packaged separately); the exact package
name depends on the distribution.
...
If everything is alright you can use `jextract`. The output below is the one I
