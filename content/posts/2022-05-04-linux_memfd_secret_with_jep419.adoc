...
title: "Using Linux's `memfd_secret` syscall from the JVM with ~~JEP-419~~ JEP-454"
summary: Linux 5.14 brought a new system call `memfd_secret` in order to mitigate speculative attack by preventing the kernel from being able to peek at these memory segments. Let's use this syscall from the JVM with the ~~JEP-419~~ JEP-454 API.
series: "Project Panama"
---

**UPDATED: 2026-04-02**

IMPORTANT: When this article was written, JEP-419 had been released as part of JDK 18, at that time the API was still
incubating and subject to change. FFM has been finalized in JDK 22 as part of JEP-454, this article has been updated
with the final public API. (https://github.com/bric3/bric3.github.io/blob/b63ae9598ef967f226278b273bad4310d9bef176/content/posts/2022-05-04-linux_memfd_secret_with_jep419.adoc[Original Article])
...
segments created by thWhen Iis system call.

In this article I will leverage the FFM API introduced by
https://openjdk.java.net/jeps/454[JEP-454] (Project Panama). This API has been
delivered as part of https://openjdk.java.net/projects/jdk/22/[JDK 22].

For those unfamiliar with it, Project Panama is a project that aims to provide an
easy, secure, and efficient way to call _native methods_ from Java. It is the
fruit of work started in 2014, from ideas even older. You can look
...
TIP: The following examples are based on *JDK 22* APIs. The Linux distribution
in the original article in 2022 was *Fedora release 35* with the kernel *5.16.20-200.fc35.x86_64*.
And later adapted to a Fedora 43 running on Orbstack on macbook M1, with a kernel *6.17.8*.

_While this article will focus on Linux, the same concepts apply to other OSes
(and CPU as we'll see)._ Also, this article is not about introducing FFM API,
this is done in other material, which means this blog assumes the right flags,
usually `--enable-native-access=ALL-UNNAMED`.

.tl;dr
**************
Here's how one can make a _syscall_ using JEP-454 since JDK 22.

[source,java]
.`memfd_secret` syscall
----
var linker = Linker.nativeLinker();
var sys_memfd_secret = linker.downcallHandle(
        linker.defaultLookup().find("syscall").orElseThrow(), // <1>
...
----
<1> Since JDK 23, you can shorten that to `linker.defaultLookup().findOrThrow("syscall")`
**************
...
And even before that, let's see what a _system call_ is.
...
To do something useful, a program has to interact with some resources,
memory, disk, network, terminal, etc. On a computer, these resources are
handled by very complex and critical software, the **O**perating **S**ystem.

To use these resources, a program has to make _system calls_ like
...
Why do system calls have to be in the kernel and not in the user space like in a
standard library? As mentioned earlier, the reasoning is that system calls are
a way to interact with, or involve, a resource like devices, file system, network,
processes, etc. These resources are managed by *privileged* software:
the OS or kernel.

When a system call happens, the program doesn't simply invoke code that resides
at some address; *a system call is actually making the CPU
 switch to _Kernel mode_* because the kernel is *privileged* software.

On most modern processors there is a security model that allows limiting the
...
When a processor executes some code (in a thread), the processor knows the current
...
_Out of scope: there are even negative rings on some CPU architectures for
hypervisor, or CPU System management, up to `Ring -3`._

The CPU enforces restrictions; in order to perform its purpose, a
user-land program needs to place a request to the kernel. This mechanism
is called *syscall*, it allows transitioning between rings.
...
The _calling convention_ of other architectures is also described e.g.,
on ARM processors, the system call is performed by a `swi 0x0` instruction,
on _aarch64_ by `svc #0`.

TIP: For people not aware of what exactly is a *calling convention* should read at least this
...
and these numbers are somewhat like _enum ordinals_ indicating the type of message.
...
Make the syscall::
* execute `svc #0` processor instruction (supervisor call)
...
NOTE: The hardware only defines `svc` as the trap mechanism — which register holds the syscall number
...
In order to quickly practice a _syscall,_ let's do a basic
...
The following is using _GNU Assembler (GAS)_ syntax (`.s` extension), which
is the way to go for AArch64, unlike NASM which is x86-focused.

[source, asm]
.hello_syscall_arm64.s (AArch64)
----
.global _start
.section .text
_start:
...
[source, asm]
.hello_syscall_macos.s (AArch64, macOS)
----
.global _start
...
From the application point of view (user land), a syscall is just like an *atomic*
...
MethodHandle mmap = linker.downcallHandle(
        ...
        FunctionDescriptor.of(
                ValueLayout.ADDRESS, // return: mapped address
...
var segmentAddress = (MemorySegment) mmap.invoke( // <1>
        MemorySegment.NULL,
        secret.length(),
...
[source, java]
----
var roSecretSegment = segmentAddress.reinterpret(length, arena, null) // <1>
                                    .copyFrom(MemorySegment.ofArray(secretBytes)) // <2>
                                    .asReadOnly(); // <3>
----
<1> Reinterpret the zero-length `MemorySegment` returned by `mmap` to the actual
size, scoped to the current `Arena`.
<2> Since `secretSegment` is actually a `MemorySegment` *off heap*, the
_source_ secret array has to be transformed first into an *on-heap* `MemorySegment`
before being copied to the target _secret_ memory mapping.
<3> Eventually make the secret segment read-only.
+
Later, to read the secret, extract the byte array from the memory segment.
+
[source,java]
----
...
needs to be aware of the _convention_ about error handling for these methods.
...
So we'll need to check the errors after each call in our case, as each of
these calls is a system call underneath.
...
.checking `errno`
----
var captureStateLayout = Linker.Option.captureStateLayout(); // <1>
...
When I wrote this article, the Linux Kernel was in the 5.x line, and tools
were not using the latest 5.14 kernel.

Fortunately, we're now in 2026, and I discovered that starting from Linux 6.5,
https://man7.org/linux/man-pages/man2/memfd_secret.2.html#:~:text=Before%20Linux%206%2E5%2C%20memfd%5Fsecret%28%29%20was%20disabled%20by%20default%20and%20only%20available%20if%20the%20system%20administrator%20turned%20it%20on%20using%20%22secretmem%2Eenable%3Dy%22%20kernel%20parameter%2E[`memfd_secret` is enabled by default],
albeit only if the Kernel is built with the https://www.kernelconfig.io/config_secretmem[CONFIG_SECRETMEM]
config option.

The good news is that, as of today, most of the distributions have it.
And the better news is that tools like Docker Desktop, or Orbstack are running a later version
of the Linux Kernel.

CAUTION: Hibernation is inhibited for as long as any `memfd_secret()` descriptions exist,
...
1. A file descriptor value is a number `4`, on Linux accessible via procfs under
`/dev/fd/4` or `/proc/self/fd/4`, but even these as a `Path` don't work.
2. And, we could not map this segment as both _write_ then _read_ via this API;
performing this operation twice, one in write-only mode and one in read-only mode,
would not work, as this special file descriptor would be closed after the first memory mapping.
...
first argument each time. ``MethodHandle``s API allows making https://codeblog.jonskeet.uk/2012/01/30/currying-vs-partial-function-application/[partial function].
...
lift most of the work required to generate the ``MethodHandle``s.
...
https://jdk.java.net/jextract/[early-access binaries] for it, so building it
from source is optional. If you want the latest bits or need to match a
specific JDK line, the https://github.com/openjdk/jextract[`jextract` project
page] explains how to build it. My test machine is a Fedora, so adapt the
command and the JDK distribution to your needs.

[NOTE]
====
As of March 26, 2026, https://jdk.java.net/jextract/[jdk.java.net/jextract]
lists the early-access build `25-jextract+2-4`, published on November 25,
2025. The repository README also states that the `master` branch targets the
latest mainline JDK, and that building from source requires a suitable JDK 23
or newer plus LLVM/libclang. In my experience a locally installed LLVM may
not fit well with the single property `llvm_home`, as LLVM and clang paths can
vary widely across Linux distributions; in practice I would rather download a
pre-built binary from https://github.com/llvm/llvm-project[llvm-project].
Note that it is huge, around 1.6 GiB at minimum.
====

[source,bash]
.Building `jextract`
----
sudo dnf install java-21-openjdk-devel # <1>
...
installation works.
<4> Finally, run the documented build command with the required JDK and LLVM home
directories.

If everything went alright, you can now use `jextract`. The output below is the one I
got from commit https://github.com/openjdk/jextract/commit/ad6430f83085c87f0f226a47c46c593d87d26376[ad6430f83085c87f0f226a47c46c593d87d26376]
...
[CAUTION]
====
When I first wrote this article, I had to specify the `errno` location,
and this also required to include `errno.h` as well. But there's a caveat:
`jextract` cannot pass custom linker options, like `Linker.Option.captureCallState("errno")`;
their guide https://github.com/openjdk/jextract/blob/ad6430f83085c87f0f226a47c46c593d87d26376/doc/GUIDE.md?plain=1#L792-L795[explicitly mentions (`ad6430f83`)]
the generated code must be manually edited.
====
...
Also, note that `jextract` supports passing the options as _option file_.
...
.Running `jextract`
----
$ jextract @memfd_secret_header.jextract.options memfd_secret_header.h # <1>
----
...
That's not https://github.com/openjdk/jextract/blob/master/doc/GUIDE.md#unsupported-features[the only unsupported thing],
...
.`memfd_secret`
Originally, I heard about this feature coming in Linux 5.14, and I was hoping to test
it after the https://en.wikipedia.org/wiki/Spectre_(security_vulnerability)[Spectre]
style attacks, at least from a developer perspective. The first thing is that you'll need a Linux
with that version. Back then it was a bit tedious to make it work since Docker Desktop
ran a Linux 5.10 kernel, on my personal machine it required me to set a kernel flag
to enable the syscall. Fortunately, those days are over, and most of the distributions
are built with that syscall, that said, better check the OS you're running.
On your regular laptop, the fact that having live descriptors **disables hibernation**
is almost a deal-breaker for this kind of hardware.
Also, personally, if an application is not having a very tight control at how secrets
are actually used after reading them from this memory, I fail to see the value
of such a feature.
...
is unpractical, but given the JVM direction to https://openjdk.org/jeps/8305968[**integrity by default**],
which affects other aspects -- including regular JNI: https://openjdk.org/jeps/472[JEP-472],
https://openjdk.org/jeps/471[JEP-471], https://openjdk.org/jeps/451[JEP-451], so I understand
...
Yet again, I'm happy to see this project Panama landing in the JDK to bridge
the gap to the native world without a third party library. I wished it was
on an LTS as folks tend to prefer those, but now that JDK 25 also landed, that should
be good enough for many projects.
...
* https://openjdk.java.net/jeps/419[JEP-424]
* https://openjdk.java.net/jeps/419[JEP-454]
...
public class MemfdSecretDemo {
  // Article section "mmap": protection and mapping flags.
  private static final int PROT_READ = 0x1;
  private static final int PROT_WRITE = 0x2;
  private static final int MAP_SHARED = 0x0001;
  private static final int MAP_PRIVATE = 0x0002;
  private static final int MAP_ANONYMOUS_LINUX = 0x0020;
  private static final int MAP_ANON_MACOS = 0x1000;

  // Article section "memfd_secret syscall": syscall number (x86_64/aarch64).
  private static final int SYS_memfd_secret = 447;
  private static final int ENOSYS_LINUX = 38; // "Function not implemented"
  private static final int ENOSYS_MACOS = 78; // kept for completeness
  private static final long MAP_FAILED = -1L;

  public static void main(String[] args) throws Throwable {
    var secret = "very-secret-portable";
    var secretBytes = secret.getBytes(StandardCharsets.UTF_8);
    var length = secretBytes.length;

    var linker = Linker.nativeLinker();

    // Article section "mmap": downcall to libc mmap.
    MethodHandle mmap = linker.downcallHandle(
        linker.defaultLookup().find("mmap").orElseThrow(),
        FunctionDescriptor.of(
    ...
    // Cleanup for both mapping strategies.
    ...
    // Article section "checking errno": capture errno in a portable way.
    MemoryLayout captureStateLayout = Linker.Option.captureStateLayout();
    long errnoOffset = captureStateLayout.byteOffset(MemoryLayout.PathElement.groupElement("errno"));
    MethodHandle syscallWithErrno = linker.downcallHandle(
        linker.defaultLookup().find("syscall").orElseThrow(),
        FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.JAVA_INT, ValueLayout.JAVA_INT),
        Linker.Option.captureCallState("errno")
    );
    // Article section "Improving our syscall API": curry syscall number -> sys_memfd_secret.
    MethodHandle sysMemfdSecretWithErrno = MethodHandles.insertArguments(
        syscallWithErrno,
        1,
        SYS_memfd_secret
    );
    MethodHandle ftruncateWithErrno = linker.downcallHandle(
        linker.defaultLookup().find("ftruncate").orElseThrow(),
        FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.JAVA_INT, ValueLayout.JAVA_LONG),
        Linker.Option.captureCallState("errno")
    );

    Mapping mapping = tryMemfdSecretOnLinux(
        mmap, close, sysMemfdSecretWithErrno, ftruncateWithErrno, captureStateLayout, errnoOffset, strerror, length
    );
    // this fallback is only for running a sanity check on non Linux OSes, or when Linux with memfd_secret is unavailable.
    if (mapping == null) {
      mapping = regularAnonymousMapping(mmap, length);
      System.out.println("mode=regular-mmap (fallback)");
    } else {
      System.out.println("mode=memfd_secret");
    }

    try {
      // Article section step 4: reinterpret and copy secret bytes to the mapped segment.
      var mapped = mapping.address.reinterpret(length);
      var readOnlySecret = mapped
          .copyFrom(MemorySegment.ofArray(secretBytes))
          .asReadOnly();

      var roundTrip = readOnlySecret.toArray(ValueLayout.JAVA_BYTE);
      if (!Arrays.equals(secretBytes, roundTrip)) {
        throw new IllegalStateException("Round-trip mismatch");
      }

      System.out.println("round-trip OK: " + new String(roundTrip, StandardCharsets.UTF_8));
    } finally {
      var rc = (int) munmap.invoke(mapping.address, (long) length);
      if (rc != 0) {
        throw new IllegalStateException("munmap failed with rc=" + rc);
      }
      if (mapping.fd != -1) {
        var closeRc = (int) close.invoke(mapping.fd);
        if (closeRc != 0) {
          throw new IllegalStateException("close failed with rc=" + closeRc);
        }
      }
    }
  }

  private static Mapping tryMemfdSecretOnLinux(
      MethodHandle mmap,
      MethodHandle close,
      MethodHandle sysMemfdSecretWithErrno,
      MethodHandle ftruncateWithErrno,
      MemoryLayout captureStateLayout,
      long errnoOffset,
    ...
    try (var arena = java.lang.foreign.Arena.ofConfined()) {
      // Article section step 1 + "memfd_secret error checking": call syscall + inspect errno.
      var captureState = arena.allocate(captureStateLayout);
      int fd = (int) sysMemfdSecretWithErrno.invoke(captureState, 0);
      if (fd == -1) {
        int errno = captureState.get(ValueLayout.JAVA_INT, errnoOffset);
        if (errno == ENOSYS_LINUX || errno == ENOSYS_MACOS) {
          System.err.println("memfd_secret unavailable or disabled (errno=ENOSYS). Fallback to regular mmap.");
        } else {
          System.err.println("memfd_secret failed (errno=" + errno + " " + errnoText(strerror, errno) + "). Fallback to regular mmap.");
        }
        return null;
      }

      // Article section step 2: ftruncate(fd, length).
      var truncateState = arena.allocate(captureStateLayout);
      int truncateRc = (int) ftruncateWithErrno.invoke(truncateState, fd, (long) length);
      if (truncateRc != 0) {
        close.invoke(fd);
        int errno = truncateState.get(ValueLayout.JAVA_INT, errnoOffset);
        System.err.println("ftruncate failed for memfd_secret fd (errno=" + errno + " " + errnoText(strerror, errno) + "). Fallback to regular mmap.");
        return null;
      }

      // Article section step 3: mmap on the secret fd.
      MemorySegment mappedAddress = (MemorySegment) mmap.invoke(
          MemorySegment.NULL,
          (long) length,
          PROT_READ | PROT_WRITE,
          MAP_SHARED,
      ...
      if (isMmapFailure(mappedAddress)) {
        close.invoke(fd);
        System.err.println("mmap on memfd_secret fd failed. Fallback to regular mmap.");
        return null;
      }

      return new Mapping(mappedAddress, fd);
    }
  }

  private static Mapping regularAnonymousMapping(MethodHandle mmap, int length) throws Throwable {
    String os = System.getProperty("os.name", "").toLowerCase();
    int anonFlag = os.contains("mac") ? MAP_ANON_MACOS : MAP_ANONYMOUS_LINUX;

    MemorySegment mappedAddress = (MemorySegment) mmap.invoke(
        MemorySegment.NULL,
        (long) length,
        PROT_READ | PROT_WRITE,
        MAP_PRIVATE | anonFlag,
        -1,
        0L
    );
    if (isMmapFailure(mappedAddress)) {
      throw new IllegalStateException("regular mmap failed");
    }
    return new Mapping(mappedAddress, -1);
  }

  private static boolean isMmapFailure(MemorySegment address) {
    long raw = address.address();
    return raw == 0L || raw == MAP_FAILED;
  }

  private static String errnoText(MethodHandle strerror, int errno) throws Throwable {
    return ((MemorySegment) strerror.invoke(errno))
        .reinterpret(256)
        .getString(0);
  }

  private record Mapping(MemorySegment address, int fd) {
  }
}
////
