...
en craftsman, on se dit qu'il faudrait écrire des tests unitaires afin de s'assurer
...
fonctionnera sans cette erreur. Le souci vient du fait que wiremock utilise un
...
* SSL est l'acronyme de **S**ecure **S**ockets **L**ayer, ce protocole a été développé
    par *Netscape* (pour ceux qui se rappellent du navigateur concurrent d'Internet
    Explorer).
...
Ce que fait TLS c'est de s'assurer que la communication entre deux peer soit sécurisée.
TLS link:https://tools.ietf.org/html/rfc5246#section-7[s'appuie notamment sur *PKIX*]
(link:https://tools.ietf.org/html/rfc5280[**P**ublic **K**ey **I**nfrastrcture **X**.509])
...
Dans ce cas précis la javadoc de la méthode link:https://docs.oracle.com/javase/8/docs/api/javax/net/ssl/SSLContext.html#init-javax.net.ssl.KeyManager:A-javax.net.ssl.TrustManager:A-java.security.SecureRandom-[`SSLContext#init`]
...
Mais en HTTPS, tous les clients doivent en fait vérifier l'identité du serveur avec
le hostname (link:https://tools.ietf.org/html/rfc2818#section-3.1[RFC 2818]) ceci
...
car cela revient à désactiver la sécurité sur toutes les connections SSL.
...
link:https://developer.android.com/training/articles/security-ssl.html#CommonHostnameProbs[documentation Android]
...
Il faut ajouter un link:https://docs.oracle.com/javase/8/docs/api/javax/net/ssl/HostnameVerifier.html[`HostnameVerifier`]
...
Cette commande donne en retour plein de données intéressantes :
...
certificat auto-signé. Plus précisément le link:https://wiki.openssl.org/index.php/Manual:Verify(1)[wiki openssl]
indique :
...
être chargées, par exemple sur une JVM Sun/Oracle `SunX509` (ce qui ne sera bien
évidement pas disponible sur IBM J9 ou sur Android. En revanche ces implémentations
sont toutes du type `X509TrustManager`.

À noter aussi le côté très générique cette API fait que `TrustManager.getTrustManagers()`
...
____
Il est également est possible de regarder le contenu du store `cacerts` :
...
Après l'importation, on y retrouvera le certificat `wiremock` à coté de
...
de ses IPs associées correspond à ce certificat (link:https://tools.ietf.org/html/rfc2818#section-3.1[RFC 2818]).

Quoi qu'il en soit cette procédure demande une modification de la configuration de
...
____
Plutôt que de rentrer _interactivement_ le mot de passe, en ajoutant
...
`OkSSLConnect` est le programme censé établir la connexion au serveur wiremock,
...
* Charger le truststore, par défaut `KeyStore.getDefauktType()` retourne
  `jks` ^link:https://docs.oracle.com/javase/8/docs/technotes/guides/security/StandardNames.html#KeyStore[[1\]]^
  ^link:https://docs.oracle.com/javase/8/docs/api/java/security/KeyStore.html#getDefaultType--[[2\]]^.
...
La javadoc de la méthode link:https://docs.oracle.com/javase/8/docs/api/java/security/cert/CertificateFactory.html#generateCertificate-java.io.InputStream-[`CertificateFactory.generateCertificate(InputStream)`]
...
On a vu le côté client, mais se baser sur le certificat de wiremock, n'est
...
car il demande de modifier le hostname verifier (link:https://tools.ietf.org/html/rfc2818#section-3.1[RFC 2818]).
Ce problème peut être corrigé en ajoutant une section link:https://tools.ietf.org/html/rfc5280#section-4.2.1.6[`SAN` (**S**ubject
**A**lternative **N**ames)]
...
elle permet de donner d'indiquer les _noms_ de serveurs pour lesquels ce certificat
...
link:https://tools.ietf.org/html/rfc5280#section-4.2.1.6[wildcards], ceci dit la logique du client
...
----
+
Cette commande créée une clé privée de 2048 bit en utilisant l'algorithme RSA.
Cette clé n'est pas protégé par mot de passe (option `-des3`).

. Faire la demande d'un certificat (façon non-interactive)
+
[source,sh]
----
openssl req \
...
----
+
Cette demande génère un fichier CSR (**C**ertificate **S**ign **R**equest) au format PEM.
Il prend la clé privée du propriétaire des serveurs au format PEM. Les informations qui
concernent la configuration de l'émission du CSR sont indiquées dans un fichier de config
(dans cette commande passée via un stream `pass:[<(cat <<-MARKER ... MARKER)]`).
+
...
link:https://www.openssl.org/docs/manmaster/man1/req.html[man (branche master)].
...
configuration ou par un stream `-extfile `pass:[<(cat <<-EOF ... EOF)]`, il faut
...
la page link:https://www.openssl.org/docs/manmaster/man1/x509.html[man (branche master)].
...
Ces trois étapes peuvent être réduites en une seule pour les certificats auto-signés.
...
* utilisation de deux points `:` avec d'un côté le type de nom alternatif
...
Bien que toutes les link:https://docs.oracle.com/javase/8/docs/technotes/guides/security/StandardNames.html#impl[implémentations de la JVM]
soient tenues d'implémenter le chargement de keystore au format PKCS12, Jetty ne
link:https://wiki.eclipse.org/Jetty/Howto/Configure_SSL#Loading_Keys_and_Certificates_via_PKCS12[supporte pas directement les fichiers p12],
...
link:https://www.bouncycastle.org/java.html[BouncyCastle] s'intègre dans ce mécanisme.
