...
Yet it is still possible to use asciidoctor'spassthrough block with the HTML
right class from FA5 `<i class="fab fa-java"></i>`
...
Also some of the blog post are sufggesting to switch to semantic HTML5, because the
asciidoctor HTML5 structure is not modern enough, e.g.
...
for me, brooken syntax highlighting, title are not styled as expected, so I didn't
use it in the end, which simplify the setup.
...
https://github.com/asciidoctor/asciidoctor-diagram/issues/247
https://github.com/asciidoctor/asciidoctor-diagram/commit/2bf075fe07dde384326e47282970832b6f7aeb1d
...
include::content/posts/2020-04-20-tackling-hugo-integration-of-asciidoctor/plantuml.smoketest.adoc[leveloffset=3]
====
