...
title: "10 or more reasons to **not** use JDK's `HttpClient`"
...
[CAUTION]
====
When reading these lines, the following is based on facts, but keep in mind the whole article represents my opinion only.
====

.tl;dr
**************
The Java `HttpClient` is a great addition to the JDK, but its API limitations and issues significantly diminish its value.
It looks like a general purpose HTTP client, but its one-size-fits-all approach is just not going to cut it.
...
* Need to work with TLS certificate pinning, hostname verification => Work with the JVM's SSL/TLS abstractions, no finer customization points
...
{{< bluesky link="https://bsky.app/profile/julien.ponge.org/post/3mhzsa5ihrc27" >}}
...
** *Update 2026-04-03*: Since this article appeared, a new https://openjdk.org/jeps/8377806[*draft* JEP 8377806]
has been submitted to add UDS support in the client. While the JEP is still a draft, it has already substantial
limitations: UDS special URL schemes are not supported, while the suggested API appear to accept a `SocketAddress`,
the only effective supported implementation will be `UnixDomainSocketAddress`, preventing anyone to actually provide
customization.
...
. No event listener to explore the behavior of HTTP calls from beginning to end. This can be done by wrapping
...
Third party libraries, however, can bring that support by wrapping the HttpClient.
...
I handpicked a few issues that I think are relevant for me. Granted, many of these below have a fix;
...
. Converting a body from gzip (`BodySubscribers.mapping`) can lead to a deadlock (because it blocks the executor thread). Fixed in JDK 13.

** https://bugs.openjdk.org/browse/JDK-8217627[JDK-8217627 HttpClient: The API documentation of BodySubscribers::mapping promotes bad behavior]
** https://bugs.openjdk.org/browse/JDK-8219213[JDK-8219213 Unable to use BodySubscribers.mapping with InputStream as mentioned in the doc]
** https://bugs.openjdk.org/browse/JDK-8217264[JDK-8217264 HttpClient: Blocking operations in mapper function do not work as documented]
...
Some wrapper libraries can help, but the underlying issues remain and can't be worked around.
...
link:{{% ref "posts/2017-10-19-self-signed-certificates-in-java.en.adoc" %}}[testing with self-signed certificates].
...
.httpclient
--
The JDK's HttpClient does not allow using a custom socket factory, which means that using a custom socket is not possible.
--
