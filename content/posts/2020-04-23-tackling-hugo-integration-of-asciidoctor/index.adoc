...
but the `asciidoctor-diagram` outputs the images in the basedir. This
can be changed by modifying the `./bin/asciidoctor` with this attribute
(https://asciidoctor.org/docs/asciidoctor-diagram/#image-output-location[doc]):
...
Diagrams will be put in the _gen folder which is used for generated content and as such
won't be suggested by a `git status`.
